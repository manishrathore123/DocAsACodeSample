...
def markdown_to_storage(md_content):
    md_content = md_content.replace('\r\n', '\n').replace('\r', '\n')
    mermaid_regex = re.compile(r'```mermaid\s*\n(.*?)\n\s*```', re.DOTALL)
    html_parts = []
    last_end = 0
    for match in mermaid_regex.finditer(md_content):
        before = md_content[last_end:match.start()]
        if before.strip():
            html_parts.append(markdown.markdown(before))
        ...
        html_parts.append(macro)
        last_end = match.end()
    remaining = md_content[last_end:]
    if remaining.strip():
        html_parts.append(markdown.markdown(remaining))
    return ''.join(html_parts)

def find_page_in_space_by_title(title):
    try:
